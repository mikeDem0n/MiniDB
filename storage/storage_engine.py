...
class StorageEngine:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        """获取存储引擎统计信息"""
        total_records = sum(meta.record_count 
                           for meta in self.table_metadata.values())
        total_pages = sum(len(meta.page_ids) 
                         for meta in self.table_metadata.values())
...
def main():
    ...
    with StorageEngine(db_path, buffer_size=8) as storage:
        # 创建表
        print("1. 创建表...")
        success = storage.create_table("student", 
                                     ["id", "name", "age"], 
                                     ["INT", "VARCHAR", "INT"])
