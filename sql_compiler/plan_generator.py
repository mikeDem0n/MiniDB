...
from .ast_nodes import (
    ASTVisitor, SQLProgram, CreateTableStatement, InsertStatement,
    SelectStatement, DeleteStatement, BinaryExpression, Identifier,
    Literal, ColumnDef, DataType
)
        ...
        for value_row in node.values:
            ...
            for value in value_row:
                if isinstance(value, Literal):
                    ...
                    raise PlanGenerationError(f"Unsupported value type in INSERT: {type(value)}", 
    ...
    def visit_select_statement(self, node: SelectStatement) -> ExecutionPlan:
        ...
        for select_item in node.select_list:
            if isinstance(select_item, Identifier):
                ...
                raise PlanGenerationError(f"Unsupported select item: {type(select_item)}", 
                                        select_item.line, select_item.column)
